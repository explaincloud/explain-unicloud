...
module.exports = async ({
	...
	// 声明参数
	let service, action, route, method = undefined;
	...
	if (isHttp) {
				...
				x.routes.forEach(xx => {
					...
					if (xx.httpMethod) {
						...
						if (actionName.toUpperCase().startsWith("GET")) {
							httpMethod = ["GET"];
						} else if (actionName.toUpperCase().startsWith("POST") ||
							actionName
		...
		routeTemplates = routeTemplates.sort((a, b) => {
			...
			if (arr.length < brr.length) {
				...
				var arsl = a.routeTemplate.replace(new RegExp("\\{[^\\}]+\\}", "g"),
					EXPLAIN_ROUTE_PARAMETER_MATCHER).split(
					EXPLAIN_ROUTE_PARAMETER_MATCHER).length - 1;
				var brsl = b.routeTemplate.replace(new RegExp("\\{[^\\}]+\\}", "g"),
					EXPLAIN_ROUTE_PARAMETER_MATCHER).split(
					EXPLAIN_ROUTE_PARAMETER_MATCHER).length - 1;
				if (arsl > brsl) {
					return true;
		...
		var repeatRouteTemplates = routeTemplates.filter(x => {
			if (matchedRouteTemplate.routeTemplate.replace(new RegExp("\\{[^\\}]+\\}", "g"),
					EXPLAIN_ROUTE_PARAMETER_MATCHER) ===
				x.routeTemplate.replace(new RegExp("\\{[^\\}]+\\}", "g"),
					EXPLAIN_ROUTE_PARAMETER_MATCHER)) {
				return x;
			}
			...
			if (x.includes(EXPLAIN_ROUTE_PARAMETER_MATCHER)) {
				if (x === EXPLAIN_ROUTE_PARAMETER_MATCHER) {
					...
					for (var i = 0; i < _f.length - 1; i++) {
						...
						var _m = new RegExp(_o, "i").exec(_r)[0].replace(new RegExp(
							`(^${_h})\\{|\\}(${_l}$)`, "gi"), '');
						var _n = new RegExp(_p, "i").exec(_g)[0].replace(new RegExp(
							`(^${_h})|(${_l}$)`, "gi"), '');
						_r = _r.substring(_r.indexOf(`${_h}{${_m}}`) + `${_h}{${_m}}`.length, _r
							.length);
						_g = _g.substring(_g.toLowerCase().indexOf(`${_h}${_n}`.toLowerCase()) +
							`${_h}${_n}`.length, _g.length);
